...
def talk_to_client():
    ...
    while True:
        if data == "ServerCreated":
            ...
            data = conn.recv(1024).decode()
            Temp_JSON = {"ip":data,"players":0,"status":3}
            Servers.append(Temp_JSON)
            conn.send("ok_now_break_conn".encode())
            continue
        #PlayerLoginInit
        elif data == "server_edit_data":
            conn.send("ip_request".encode())
            data = conn.recv(1024).decode()
            conn.send("status_num_request".encode())
            data = conn.recv(1024).decode()
            for i in Servers:
                if i["ip"] == data:
                    i["status"] = int(data)
            ...
            if os.path.isfile("./player_data/"+data):
                ...
                data = f.read()
                f.close()
                if data["money"] >= Item_Data[data2]:
                    f.open("./player_data/"+data,"w")
                    data["Inventory"].append(data2)
                    data["money"]-=Item_Data[data2]
                    f.write(data)
                    f.close()
                    conn.send("ok".encode())
                else:
